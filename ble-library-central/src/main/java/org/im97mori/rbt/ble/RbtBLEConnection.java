...
public class RbtBLEConnection extends BLEConnection {
     ...
     */
    public Integer createRbtWriteCharacteristicTask(UUID serviceUUID, UUID characteristicUUID, ByteArrayInterface byteArrayInterface, int waitTarget, long timeout, @Nullable Bundle argument, @Nullable RbtCallback rbtCallback) {
        Integer taskId = null;
        BluetoothGatt bluetoothGatt = mBluetoothGatt;
        if (bluetoothGatt != null) {
            RbtWriteCharacteristicTask task = new RbtWriteCharacteristicTask(this, bluetoothGatt, mTaskHandler, serviceUUID, characteristicUUID, byteArrayInterface, waitTarget, timeout, BLECallbackDistributer.wrapArgument(argument, rbtCallback));
            mTaskHandler.addTask(task);
            taskId = task.getTaskId();
        }
        return taskId;
